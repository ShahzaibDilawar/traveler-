...
<?php else: ?>
<?php 
	if ($_REQUEST['comment'] AND $_REQUEST['post_id']) {
		# code...
		$comment = mysqli_real_escape_string($dbc,strip_tags($_REQUEST['comment']));
		if (mysqli_query($dbc,"INSERT INTO comments(user_id,post_id,comment) VALUES('$fetchUser[id]','$_REQUEST[post_id]','$comment')")) {
			# code...
			?>
				<?php $getPost = mysqli_query($dbc,"SELECT * FROM post ORDER BY id DESC");
  ...
  $user = fetchById($dbc,"users",$val);
	...
	?>
<script>window.location="feeds.php"</script>
	<?php
	}
 ?>
<?php endif; ?>
